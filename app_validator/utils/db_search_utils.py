...
class FaissSearchEngine:
    ...
    def build_index(self, csv_path: str, index_path: str, meta_path: str):
        ...
        Args:
            csv_path (str or Path): 원본 데이터 CSV 파일 경로.
            index_path (str or Path): 생성된 FAISS 인덱스를 저장할 경로.
            meta_path (str or Path): 추출된 메타데이터(pickle)를 저장할 경로.
        """
        # Path 객체를 문자열로 변환
        ...
        print(f"✅ 인덱스 및 메타데이터 저장이 완료되었습니다. ('{index_path_str}', '{meta_path_str}')", flush = True)
    ...
    def load_index(self, index_path: str, meta_path: str):
        ...
        Args:
            index_path (str or Path): FAISS 인덱스 파일(.index) 경로.
            meta_path (str or Path): 메타데이터 파일(.pkl) 경로.
        """
        # Path 객체를 문자열로 변환
        index_path_str = str(index_path)
        meta_path_str = str(meta_path)
        ...
        if not (os.path.exists(index_path_str) and os.path.exists(meta_path_str)):
            raise FileNotFoundError(f"인덱스 파일 '{index_path_str}' 또는 메타데이터 파일 '{meta_path_str}'을 찾을 수 없습니다.")
            
        print(f"'{index_path_str}' 및 '{meta_path_str}'에서 인덱스와 메타데이터를 로드합니다...", flush = True)
